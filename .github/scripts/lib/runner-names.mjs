/**
 * Shared helpers for the qvac-ext-marian-dev runner-label catalog.
 *
...
// use nothing but ubuntu-latest / macos-latest / the org reusable (arm, macos,
// wasm-*, native-customized_marian-macos, check-approvals, security-baseline)
// stay out of scope.
const ADDON_WORKFLOWS = new Set(['ubuntu.yml', 'windows.yml', 'release.yml'])
...
export function findHardcodedLabelViolations(relativePath, source, runners) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // `matrix.os == 'ubuntu-20.04'` are not mistaken for a hardcoded runner label.
    if (value.includes('${{')) {
